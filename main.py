...
class fun(webapp2.RequestHandler):
    def get(self):
        template_values = {}
        template = jinja_environment.get_template('hw.html')
        self.response.out.write(template.render(template_values))

class notfun(webapp2.RequestHandler):
    def get(self):
        template_values = {}
        template = jinja_environment.get_template('fun.xml')
        self.response.out.write(template.render(template_values))

class lovely(webapp2.RequestHandler):
    def get(self):
        template_values = {
            'name': 'Joe',
